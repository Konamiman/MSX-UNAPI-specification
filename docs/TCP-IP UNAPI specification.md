...
or zero to skip server certificate validation (only when using TLS on an active connection)  
...
* The connection is active
  ...
  * A non-zero value is provided in the "server host name" field of the connection parameters block: this value is to
  be interpreted as the address of a zero-terminated string that represents the host name
  of the server. The implementation must check the validity of the certificate provided by the server,
  and if there is any error (for example the host name in the certificate doesn't match the name
  supplied in the parameters block), then the connection must be closed immediately.

  * Zero is provided in the "server host name" field of the connection parameters block: the implementation must not
...
the value of the "server host name" field of the connection parameters block, acting as if it was zero.
...
* 10: TLS: The host name in the server certificate didn't match the name provided in the parameters block when opening the connection.
* 11: TLS: The server certificate has expired.
* 12: TLS: Error in the certificate chain.
* 13: TLS: Our certificate was rejected by the peer.
* 14: TLS: Other error.
